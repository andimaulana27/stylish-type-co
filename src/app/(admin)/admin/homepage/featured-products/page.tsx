// src/app/(admin)/admin/homepage/featured-products/page.tsx
'use client';
...
const FeaturedProductsManager = ({ initialProductIds }: { initialProductIds: string[] }) => {
    ...
    const sectionDescription = "Select up to 8 fonts to feature. Only fonts with the 'Staff Pick' label are available here.";
    const sectionKey = "featured_products";
    const productType = "font";
    const limit = 8;
    const staffPickOnly = true;
...
export default function ManageFeaturedProductsPage() {
    ...
    useEffect(() => {
        getHomepageConfigAction().then(homepageConfig => {
            const featuredConfig = homepageConfig.find(s => s.section_key === 'featured_products');
            setConfig({ product_ids: featuredConfig?.product_ids || [] });
            setLoading(false);
        });
    }, []);
    ...
    return (
        <div className="space-y-8">
            <div>
                <h1 className="text-3xl font-bold text-brand-light">Manage Featured Products</h1>
                <p className="text-brand-light-muted">Control the &quot;Featured Products&quot; section on your homepage.</p>
            </div>
            {loading && <div className="p-6 bg-brand-darkest rounded-lg border border-white/10 animate-pulse h-96"></div>}
            {!loading && config && <FeaturedProductsManager initialProductIds={config.product_ids} />}
        </div>
    );
}